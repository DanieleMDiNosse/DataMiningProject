...
Un classificatore di tipo $Decision Tree$ è caratterizzato da un diverso numero di parametri. Per la loro scelta è stata utilizzata la cross-validation in modo da ottenere la combinazione più performante deglli stessi. 
I parametri scelti e il relativo range sono i seguenti:

\begin{itemize}
\item min impurity decrese $ \in [0,0.2]$: il minimo gain dell'impurità affinché il nodo venga splittato;
...
La migliore combinazione ottenuta risulta essere la seguente:\\
\begin{itemize}
\item min impurity decrese$=$; max depth$=$; min samples split$=$; min samples leaf$=$; $\alpha$=$0$.
\end{itemize}
